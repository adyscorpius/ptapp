...
(function() {
  ...
  util.percentHanging = 0.5; // --- percentage of items needed to appear to hang before a warning is given
  util.hangWarningMessage =
    "{n} Prices seem to be stagnant... did the bot hang?"; // --- hang message
  //=========================================
  //===========  END SETTINGS  ==============
  //=========================================

  util.msPerDataFrame = 12900; //assumed average time
  util.topOffsetPercentage = Math.min(
    60,
    Math.max(2, util.topOffsetPercentage)
  );
  util.bottomOffsetPercentage = Math.min(
    60,
    Math.max(2, util.bottomOffsetPercentage)
  );
  util.graphFrames = ((util.graphMinutes || 5) * 6) >> 0;
  util.hangThreshold = Math.min(999, Math.max(1, util.hangThreshold));
  util.percentHanging = Math.min(1, Math.max(0.001, util.percentHanging));
  ...
  util.graph.prototype.setSelector = function(selector) {
    ...
    if (canvas.length < 1) {
      $("body").append(
        '<div style="position:absolute;display:none;"><canvas id="myCanvas"></canvas></div>'
      );
      var canvas = $("#myCanvas");
  ...
  util.graph.prototype.drawStats = function() {
    ...
    if (size.width != util.canvas.width || size.height != util.canvas.height) {
      util.canvas = util.createHiDPICanvas(
        size.width,
        size.height,
        1,
        $("#myCanvas")[0]
    ...
    if (util.drawZeroLine && this.drawZero) {
      ...
      ctx.fillText("0%", 0, ((percent * size.height) >> 0) + 0.5);
      ctx.beginPath();
      ctx.moveTo(20, ((percent * size.height) >> 0) + 0.5);
      ctx.lineTo(size.width, ((percent * size.height) >> 0) + 0.5);
      ctx.stroke();
    }
    ...
    if (util.drawSellThreshold && this.drawProfit) {
      ...
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(0, ((percent * size.height) >> 0) + 0.5);
      ctx.lineTo(size.width, ((percent * size.height) >> 0) + 0.5);
      ctx.stroke();
    }
    ...
    for (var i = 0; i < totalRun; i++) {
      if (this.stats.data[i] != null && this.stats.data[i] != "") {
        if (first) {
          first = false;
          ctx.moveTo(
            index / totalRun * size.width,
            size.height -
              (this.stats.data[i] - range.min) / range.size * size.height
          );
        } else {
          ctx.lineTo(
            index / totalRun * size.width,
  ...
  function tick(data) {
    ...
    for (var i = 0; i < dataTypes.length; i++) {
      ...
      for (var j = 0; j < source.length; j++) {
        ...
        if (pairData[pair] == undefined) {
          pairData[pair] = {
            ...
            graph: new util.graph(
              containers[dataTypes[i]].drawZero,
              containers[dataTypes[i]].drawProfit
            )
          };
        ...
        setCacheData(
          pair,
          pairData[pair].graph.stats.data,
          pairData[pair].lastTick
        );
    ...
    if (
      util.testHangWarning &&
      hangStats.max > 0 &&
      hangStats.signaled / hangStats.max >= util.percentHanging
    ) {
    ...
    for (var i = 0; i < renderTypes.length; i++) {
      ...
      if (curParent.width() != 100) {
        ...
        for (var j = 0; j < curParent.length; j++) {
          ...
          if (cur !== undefined) {
            //we can render it!
            cur.graph.setSelector(
              $(curParent[j]).children("." + curContainer.childDestination)
            );
            cur.graph.drawStats();
          }
        }
  ...
  $(".dca-log,.dust-log,.pairs-log,.possible-buys-log,.pending-log").on(
    "click",
    function() {
      setTimeout(function() {
        render();
  ...
  $("table td.market.all a").each(function() {
    ...
    $(this).attr(
      ...
      value.replace(
        ...
        "www.binance.com/tradeDetail.html"
      )
    );
...
$(
  ".dca-log, .pairs-log, .dust-log, .sales-log, .pending-log, .possible-buys-log"
).on("click", function() {
  setTimeout(function() {
    binanceAdvancedExchange();
  }, 100);
...
function estimateYesterdayPercent() {
  ...
  if ($("#mYesterdayProfit").text() !== "") {
    if ($("#mYesterdayProfitPCTValue").text() === "") {
      $("span.market-price-calculations.text-profityd").append(
        '<br><label class="usd-value"><span class="full-text">Estimated Percent Gain&nbsp;</span><span class="short-text">Est. % Gain&nbsp;</span></label><span class="mb-0  main-text" id="mYesterdayProfitPCTValue" title="' +
          prevPercent +
          '">' +
          prevPercent +
          "</span>"
...
function estimatePercent() {
  var todayPercentCalc = (
    $("#mTodayProfit").text() /
    $("#mTotalCurrentVal").text() *
    100
  ).toFixed(2);
  ...
  if ($("#mTodayProfit").text() !== "") {
    if ($("#mTodayProfitPCTValue").text() === "") {
      $("span.market-price-calculations.text-profittd").append(
        '<br><label class="usd-value"><span class="full-text">Estimated Percent Gain&nbsp;</span><span class="short-text">Est. % Gain&nbsp;</span></label><span class="mb-0  main-text" id="mTodayProfitPCTValue" title="' +
          todayPercent +
          '">' +
          todayPercent +
          "</span>"
...
function estimate() {
  ...
  if ($("#dtDcaLogs thead").length > 0) {
    if ($("#dtDcaLogs thead .est-usd").length < 1) {
      $("#dtDcaLogs thead tr").append(
        '<th class="text-right est-usd all sorting" rowspan="1" colspan="1" style="width: 92px;">Estimated Value</th>'
      );
    }
    $("#dtDcaLogs tbody tr").each(function() {
      ...
      if (difference > 0) {
        $(this)
          .find(".est-usd")
          .html(
            '<span style="color:#05b16f;"><i style="color:#98a6ad;font-style:normal;">$' +
              total +
              "</i><br>$" +
              difference +
              "</span>"
        ...
        $(this)
          .find(".est-usd")
          .html(
            '<span style="color:#d85353;"><i style="color:#98a6ad;font-style:normal;">$' +
              total +
              "</i><br>$" +
              difference +
              "</span>"
    ...
    var sta = val * btc1;
    var total = sta.toFixed(2);
    $("#dcLogDifference").prepend(
      '<b style="color:#98a6ad;font-weight:400;margin-right:5px">($' +
        difference +
        ")</b>"
    );
    $("#dcLogTotalCurrentVal").prepend(
      '<b style="color:#98a6ad;font-weight:400;margin-right:8px">($' +
        total +
        ")</b>"
    );
    $("#dcLogRealCost").prepend(
      '<b style="color:#98a6ad;font-weight:400;margin-right:8px">($' +
        bought +
        ")</b>"
    );
  ...
  if ($("#dtPairsLogs thead").length > 0) {
    ...
    var sta = val * btc1;
    var total = sta.toFixed(2);
    $("#pairsLogDifference").prepend(
      '<b style="color:#98a6ad;font-weight:400;margin-right:5px">($' +
        difference +
        ")</b>"
    );
    $("#pairsLogTotalCurrentVal").prepend(
      '<b style="color:#98a6ad;font-weight:400;margin-right:8px">($' +
        total +
        ")</b>"
    );
    $("#pairsLogRealCost").prepend(
      '<b style="color:#98a6ad;font-weight:400;margin-right:8px">($' +
        bought +
        ")</b>"
    );
  ...
  if ($("#dtDustLogs thead").length > 0) {
    ...
    var sta = val * btc1;
    var total = sta.toFixed(2);
    $("#dustLogDifference").prepend(
      '<b style="color:#98a6ad;font-weight:400;margin-right:5px">($' +
        difference +
        ")</b>"
    );
    $("#dustLogTotalCurrentVal").prepend(
      '<b style="color:#98a6ad;font-weight:400;margin-right:8px">($' +
        total +
        ")</b>"
    );
    $("#dustLogRealCost").prepend(
      '<b style="color:#98a6ad;font-weight:400;margin-right:8px">($' +
        bought +
        ")</b>"
    );
  ...
  if ($("#dtSalesLog thead").length > 0) {
    ...
    var sta = val * btc1;
    var total = sta.toFixed(2);
    $("#salesLogDifference").prepend(
      '<b style="color:#98a6ad;font-weight:400;margin-right:5px">($' +
        difference +
        ")</b>"
    );
    $("#salesLogTotalCurrentVal").prepend(
      '<b style="color:#98a6ad;font-weight:400;margin-right:8px">($' +
        total +
        ")</b>"
    );
    $("#salesLogBoughtCost").prepend(
      '<b style="color:#98a6ad;font-weight:400;margin-right:8px">($' +
        bought +
        ")</b>"
    );
  ...
  $("#nBalanceVal").append(
    '<span style="display:inline-block;position:absolute;bottom:-8px;right:0;font-size: 10px">($' +
      total +
      ")</span>"
  );
  //Current
...
var ConvertLogDates = {
  init: function() {
    var _parent = this;
    $("body").on("DOMSubtreeModified", "#dvLastUpdatedOn", function() {
      if ($("#dtDcaLogs").text() !== "") _parent.convertDCADates();

      if ($("#dtPairsLogs").text() !== "") _parent.convertPairsDates();

      if ($("#dtSalesLog").text() !== "") _parent.convertSalesLogDates();

      if ($("#dtDustLogs").text() !== "") _parent.convertDustLogDates();
    });
    ...
    $(document).on(
      "click",
      ".dca-log, .pairs-log, .dust-log, .sales-log, th.date.all, .page-link, .page-item, .sorting_asc, .sorting_desc",
      function() {
        setTimeout(function() {
          if ($("#dtSalesLog").text() !== "") _parent.convertSalesLogDates();
          if ($("#dtDcaLogs").text() !== "") _parent.convertDCADates();
  ...
  convertLocalTimeToAMPM: function() {
    var time = this.toDate($("#dvCurrentTime").text(), "h:m"),
      currentServerLocalAMPMTime = time.toLocaleString("en-US", {
        hour: "numeric",
        minute: "numeric",
        hour12: true
      });
    ...
    if ($("#dvCurrentUTCTime").text() !== "") {
      if ($("#dvCurrentAMPMTime").text() === "") {
        $("#dvCurrentTime").after(
          "<span id='dvCurrentAMPMTime'>" +
            currentServerLocalAMPMTime +
            "</span>"
        );
      } else {
